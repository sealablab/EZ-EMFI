# BAD Phase 3: Register Interface Package (Revision 2)
...
**Revision 2 Changes:**
- Corrected DS1140_PD example to use only `BasicAppDataTypes` (no legacy `unsigned_8`)
- Clarified voltage values use millivolts (human-friendly units)
- Added proper DS1120A probe context (50ns fixed pulse width)
- Implemented TypeConverter dispatcher for raw value conversion
- Removed all references to non-existent `TypeConverter.to_raw()` method
...
libs/basic-app-datatypes/
  ...
  ✅ TypeConverter (voltage-specific conversion methods)
...
- [ ] `models/custom_inst/reg_package.py` implemented
  - [ ] DataTypeSpec with UI metadata
  - [ ] BasicAppsRegPackage with mapping integration
  - [ ] `_convert_to_raw()` dispatcher method working
  - [ ] `to_control_registers()` export method
  - [ ] YAML serialization (to_yaml/from_yaml)

- [ ] Legacy code deleted (clean break)
  - [ ] models/custom_inst/app_register.py removed
  - [ ] models/custom_inst/custom_inst_app.py removed
...
- [ ] Tests passing
  - [ ] DataTypeSpec validation tests
  - [ ] BasicAppsRegPackage tests
  - [ ] YAML roundtrip tests
  - [ ] MokuConfig integration tests

- [ ] Documentation
  - [ ] Phase summary written (BAD_Phase3_COMPLETE.md)
...
4. **TypeConverter dispatcher pattern**
   - No generic `to_raw()` method exists
   - Dispatcher routes to voltage-specific methods
   - Time/boolean types already in raw format

5. **Human-friendly units throughout**
   - Voltages in millivolts (2400 = 2.4V)
   - Times in nanoseconds/milliseconds
   - No hex values in YAML

6. **Export to moku-models**
   - to_control_registers() bridges to SlotConfig
   - Compatible with existing deployment tools
   - No changes needed to tools/moku_go.py

7. **Clean break from legacy**
